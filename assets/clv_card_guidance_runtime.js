...
// V4: lightweight, non-blocking, no MutationObserver loops.
(function(){
  'use strict';
  if (window.__baClvCardGuidanceV4) return;
  window.__baClvCardGuidanceV4 = true;

  // Stop older heavy observers if they exist on the current page after a hot reload.
  try {
  ...
  function style(){
    if (document.getElementById('ba-clv-guidance-style-v4')) return;
    var css = [
      '.ba-clv-guidance{display:block!important;margin:10px 0 12px!important;padding:10px 12px!important;border-radius:16px;border:1px solid rgba(255,255,255,.12);background:rgba(11,18,32,.78);box-shadow:0 10px 24px rgba(0,0,0,.18);font-family:var(--font-sans,system-ui,sans-serif);pointer-events:none}',
    ...
    var el = document.createElement('style'); el.id = 'ba-clv-guidance-style-v4'; el.textContent = css; document.head.appendChild(el);
  }
  function profile(row){
    var v = String(row.verdict || '');
    if (v === 'PROFITABIL_CONFIRMAT') return {icon:'✓', title:'Piață CLV confirmată', sev:'good', action:'Semnal istoric sănătos. Stake normal.'};
    ...
    if (v === 'SAMPLE_MIC') return {icon:'i', title:'Sample insuficient', sev:'info', action:'Prea puține pick-uri settle-ate.'};
  ...
  function makeBadge(row, mk){
    var p = profile(row), sev = 'ba-clv-' + (p.sev || row.severity || 'info'), adj = Number(row.edge_adjustment_pp || 0);
    var action = p.action || row.action || '';
    if (adj > 0) action += ' Prag recomandat: edge +' + adj.toFixed(1) + 'pp.';
    var box = document.createElement('div'); box.className = 'ba-clv-guidance ' + sev;
    box.innerHTML = '<div class="ba-clv-head"><div class="ba-clv-left"><div class="ba-clv-ico">'+p.icon+'</div><div><div class="ba-clv-kicker">Indicator piață</div><div class="ba-clv-title">'+p.title+'</div></div></div><div class="ba-clv-pill">'+marketLabel(mk)+'</div></div><div class="ba-clv-grid"><div class="ba-clv-metric"><span>CLV med</span><b>'+pct(row.avg_clv_pct)+'</b></div><div class="ba-clv-metric"><span>ROI piață</span><b>'+pct(row.roi_flat_pct)+'</b></div><div class="ba-clv-metric"><span>Sample</span><b>N='+(row.n || 0)+'</b></div></div><div class="ba-clv-action"><b>Ghid:</b> '+action+'</div>';
    return box;
  }
  function add(container, row, mk){
    ...
    if (marketTitle && marketTitle.parentNode && container.contains(marketTitle)) marketTitle.parentNode.insertBefore(box, marketTitle.nextSibling);
  ...
  function hook(){
    if (typeof window.renderMatches === 'function' && !window.renderMatches.__baClvGuidanceV4) {
      ...
      window.renderMatches.__baClvGuidanceV4 = true;
    }
    if (typeof window.switchTab === 'function' && !window.switchTab.__baClvGuidanceV4) {
      ...
      window.switchTab.__baClvGuidanceV4 = true;
    }
  }
