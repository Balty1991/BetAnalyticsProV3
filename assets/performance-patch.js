...
(function(){
  ...
  function installWeekHistory(){
    ...
    function html(){var v=viewData(),a=v.filter(function(x){return x.key===active})[0]||v[0],tabs=v.map(function(x){return '<button type="button" class="dash-week-tab '+(x.key===a.key?'active':'')+'" data-week-key="'+x.key+'">'+esc(x.tab)+'</button>'}).join(''),pills=a.rows.map(function(r){var ok=r.status==='win',label=(r.home&&r.away)?team(r.home)+' - '+team(r.away):(r.match||r.market||'—');return '<span class="dash-week-pill '+(ok?'dash-week-w':'dash-week-l')+'"><span class="dash-week-badge">'+(ok?'W':'L')+'</span><span class="dash-week-label">'+esc(label)+'</span><span class="dash-week-mkt">'+esc(r.market||r.bet||r.pick||'')+'</span></span>'}).join(''),m=a.m;return '<div class="dash-week-strip" id="dash-week-strip"><div class="dash-week-head"><div class="dash-week-title">📅 ULTIMELE 7 ZILE</div><div class="dash-week-sub">'+esc(a.full)+'</div></div><div class="dash-week-tabs">'+tabs+'</div><div class="dash-week-stats"><div class="dash-week-stat"><span>Evenimente</span><b>'+m.n+'</b></div><div class="dash-week-stat"><span>Win/Jucate</span><b>'+m.w+'/'+m.n+'</b></div><div class="dash-week-stat"><span>ROI</span><b style="color:'+(m.n?(m.roi>=0?'var(--grn)':'var(--red)'):'var(--txt)')+'">'+(m.n?sign(m.roi,'%'):'—')+'</b></div><div class="dash-week-stat"><span>Winrate</span><b>'+(m.n?m.wr.toFixed(1)+'%':'—')+'</b></div></div>'+(pills?'<div class="dash-week-scroll-wrap"><div class="dash-week-scroll">'+pills+'</div></div>':'<div class="dash-week-empty">Nu există evenimente finalizate pentru selecția aleasă.</div>')+'</div>'}
    function mount(){document.querySelectorAll('.dash-yday-strip').forEach(function(e){e.remove()});var perf=document.querySelector('.dashboard-v16-performance');if(!perf)return;var old=document.getElementById('dash-week-strip'),cat=perf.querySelector('.dash-cat-table-wrap');if(old){old.outerHTML=html();return}if(cat)cat.insertAdjacentHTML('afterend',html());else perf.insertAdjacentHTML('beforeend',html())}
    document.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('.dash-week-tab');if(!b)return;active=b.getAttribute('data-week-key')||'d1';mount()});
    mount();[500,1200,2500,4500].forEach(function(ms){setTimeout(mount,ms)});setInterval(mount,2000);
  }
