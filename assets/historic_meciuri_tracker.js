...
//
// 7 Zile   → 7 butoane individuale (zi cu zi, click = stats acea zi)
// Saptamani→ luna selectabila + toate sapt. calendaristice (ISO Lun-Dum)
//             din acea luna (4-5 sapt.). Sapt. curenta (incompleta) apare
//             cu indicator ⏳ si afiseaza si pending.
// Luna / An→ dropdown
// Click categorie → drill-down W/L + pending per zi
// ═══════════════════════════════════════════════════════════════════════
(function () {
  'use strict';
  if (window.__batHistoricTrackerV4) return;
  window.__batHistoricTrackerV4 = true;
  ...
  /* ────────────────────────────────────────────────────────────────────
     ...
     selDay    : 0=azi … 6                            (days7)
     weeksMonth: { y, m } — luna ale carei sapt. se arata (weeks)
     selWeekIdx: index in lista getWeeksForMonth()     (weeks)
     month     : { y, m }                             (month)
     year      : Number                               (year)
  ──────────────────────────────────────────────────────────────────── */
  var _ini = new Date();
  var S = {
    ...
    selWeekIdx: -1,    // -1 = auto (sapt. curenta)
  ...
  /* ────────────────────────────────────────────────────────────────────
     CALENDAR WEEK HELPERS  (Luni = prima zi)
  ──────────────────────────────────────────────────────────────────── */

  // Prima zi Luni a saptamanii in care cade data d
  function weekMonday(d) {
    var x = new Date(d); x.setHours(0,0,0,0);
    var dow = x.getDay(); // 0=Dum
    var toMon = dow === 0 ? 6 : dow - 1;
    x.setDate(x.getDate() - toMon);
    return x;
  }

  // Toate saptamanile calendaristice (Lun-Dum) care au cel putin o zi in luna (y,m)
  function getWeeksForMonth(y, m) {
    var firstDay = new Date(y, m, 1);
    var lastDay  = new Date(y, m + 1, 0);
    var cur = weekMonday(firstDay); // Lunea primei sapt.
    var weeks = [];
    while (cur <= lastDay) {
  ...
  // Index-ul saptamanii curente in lista, sau ultimul daca nu e gasit
  ...
  // Returneaza true daca saptamana w este cea curenta (incompleta)
  function isCurrentWeek(w) {
    var now = Date.now();
    return now >= w.s && now <= w.e;
  }
  ...
  function getPending() {
    if(typeof window.getHistory21LivePendingRows==='function'){
      try{ return window.getHistory21LivePendingRows().map(function(r){ return Object.assign({},r,{_st:'pending',source:'live'}); }); }
      catch(e){}
    }
    return [];
  }
  ...
  function getAllRows() {
    ...
    settled.forEach(function(r){ var k=String(r.event_id||'')+'::'+r.market_key; seen[k]=1; out.push(r); });
    pending.forEach(function(r){ var k=String(r.event_id||r.eventId||'')+'::'+r.market_key; if(!seen[k]){seen[k]=1;out.push(r);} });
  ...
  ═══════════════════════════════════════════════════════════════════ */
  function injectCss() {
    if(document.getElementById('bat-hist-v4-css')) return;
    var css=[
      ...
      '.bh-wkbtn.current.on{background:rgba(245,158,11,.12);border-color:rgba(245,158,11,.5);color:var(--yel)}',
      ...
      '.bh-pend-row{opacity:.75;border-left:2px solid rgba(245,158,11,.4);padding-left:7px}',
    ...
    var s=document.createElement('style'); s.id='bat-hist-v4-css'; s.textContent=css;
  ...
  function renderPeriodBar() {
    ...
    /* ── SAPTAMANI: luna selectabila + toate sapt. calendaristice ISO ── */
    if (S.mode === 'weeks') {
      // Dropdown luna
      ...
      h += '<div class="bh-wm-row">'+
             '<span class="bh-wm-lbl">Luna:</span>'+
             '<select class="bh-sel" onchange="window.batH.setWeeksMonth(this.value)">'+mopts+'</select>'+
           '</div>';

      // Butoane saptamani din luna selectata
      var weeks = getWeeksForMonth(S.weeksMonth.y, S.weeksMonth.m);
      ...
      weeks.forEach(function(w, i){
        ...
        var on2   = i === activeIdx ? ' on' : '';
        var cur   = isNow ? ' current' : '';
        var pend  = isNow ? '<span class="bh-wk-pend">\u23F3</span>' : '';
        h += '<button class="bh-wkbtn'+on2+cur+'" onclick="window.batH.week('+i+')">'+
               fmtDM(w.startDate)+' \u2013 '+fmtDM(w.endDate)+pend+
             '</button>';
      });
      h += '</div>';
    }
  ...
  function renderSummary(all) {
    ...
    var nd=s.settled===0;
    var isCurWeek = S.mode==='weeks' && (function(){
      var w=activeWeekBounds(); return isCurrentWeek(w);
    }());
    return '<div class="bh-sum">'+
      '<div class="bh-stitle">Toate \u00B7 <span style="color:var(--acc)">'+esc(periodLabel())+'</span>'+
        (isCurWeek?'<span class="bh-ptag">\u23F3 Sapt. in curs</span>':'')+
    ...
    var cards=CATS.filter(function(c){return c.key!=='all';}).map(function(cat){
      ...
      return '<div class="bh-card" onclick="window.batH.drill(\''+cat.key+'\')" style="border-color:'+bCol+'">'+
        ...
        '<div class="bh-card-meta">'+
          ...
          (s.pending>0?' <span style="color:var(--yel);opacity:.8">+'+s.pending+'\u23F3</span>':'')+
  ...
  function renderDrilldown(all) {
    ...
    // Meciurile pending (neincepute) apar la sfarsit in fiecare zi
    ...
    var groups=dayOrder.map(function(k){
      ...
      var mhtml=day.rows.map(function(r){
        ...
        if(isPend&&(r.event_date||r.date)){
          try{
            var kd=new Date(r.event_date||r.date);
            if(isFinite(kd))kickoff=' · '+kd.toLocaleTimeString('ro-RO',{hour:'2-digit',minute:'2-digit'});
          }catch(e){}
        }
  ...
  window.batH = {
    mode: function(m){
      ...
      if(m==='weeks'){ S.selWeekIdx=-1; } // auto = sapt. curenta
    ...
    setWeeksMonth: function(v){
      var p=v.split('-'); S.weeksMonth={y:parseInt(p[0]),m:parseInt(p[1])};
      S.selWeekIdx=-1; // reset la sapt. curenta (sau ultima) din luna noua
      _last=''; render();
    },
