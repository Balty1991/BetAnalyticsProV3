...
window.renderApiHistory=function(){ensureUI();var top=$('apih-top'),root=$('apih-root');if(!top||!root)return;var s=window.API_HISTORY_SUMMARY||null;var leagues=Array.isArray(window.API_HISTORY_LEAGUES)?window.API_HISTORY_LEAGUES:[];if(!s&&!leagues.length){root.innerHTML='<div class="empty-state">Nu exista inca date API history.</div>';return;}top.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px">'+
 '<div class="metric"><div class="label">Sezoane</div><div class="value">'+Number((s&&s.seasons_total)||leagues.reduce(function(a,x){return a+Number(x.seasons_count||0)},0)).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Ligi</div><div class="value">'+Number((s&&s.leagues_total)||leagues.length).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Start</div><div class="value">'+safe((s&&s.historical_coverage_start_year)||'—')+'</div></div>'+
 '<div class="metric"><div class="label">End</div><div class="value">'+safe((s&&s.historical_coverage_end_year)||'—')+'</div></div>'+
 '</div><div style="font-size:12px;color:var(--muted);margin-top:10px">Catalog brut generat din /api/seasons/. Urmatorul pas este legarea pe evenimente istorice per sezon si apoi dataset-ul pentru training.</div>';
 var cards=leagues.slice(0,34).map(function(r){return '<div class="list-item" style="margin-top:8px"><div style="font-weight:800">'+safe(r.league)+'</div><div style="font-size:12px;color:var(--muted)">Sezoane '+Number(r.seasons_count||0)+' • ani '+safe(r.first_year)+' → '+safe(r.last_year)+'</div><div style="font-size:12px;color:var(--muted)">Ultim sezon '+safe(r.latest_season_name||'—')+' • ID '+safe(r.latest_season_id||'—')+'</div></div>';}).join('');root.innerHTML=cards||'<div class="empty-state">Nu exista ligi istorice.</div>';};
