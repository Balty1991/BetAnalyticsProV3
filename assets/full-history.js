...
window.API_EVENTS_HISTORY_SUMMARY=window.API_EVENTS_HISTORY_SUMMARY||null;
window.TRAINING_DATASET_SUMMARY=window.TRAINING_DATASET_SUMMARY||null;
...
async function loadExtraHistoryData(){
 ...
 try{window.API_EVENTS_HISTORY_SUMMARY=await fetchJsonFallback(['./data/api_events_history_summary.json','data/api_events_history_summary.json','/BetAnalyticsProV3/data/api_events_history_summary.json','/data/api_events_history_summary.json'],null);}catch(e){window.API_EVENTS_HISTORY_SUMMARY=null;}
 try{window.TRAINING_DATASET_SUMMARY=await fetchJsonFallback(['./data/training_dataset_summary.json','data/training_dataset_summary.json','/BetAnalyticsProV3/data/training_dataset_summary.json','/data/training_dataset_summary.json'],null);}catch(e){window.TRAINING_DATASET_SUMMARY=null;}
}
...
function patch(){
 if(typeof window.doRefresh==='function'&&!window.__fh_refresh){
  var o=window.doRefresh;window.doRefresh=async function(){
   ...
   try{ensureUI();if($('tab-istoricfull')&&$('tab-istoricfull').classList.contains('active'))renderFullHistory();if($('tab-apihistory')&&$('tab-apihistory').classList.contains('active'))renderApiHistory();if($('tab-traininglab')&&$('tab-traininglab').classList.contains('active'))renderTrainingLab();}catch(e){}
   return r
  };window.__fh_refresh=1;
 }
 if(typeof window.switchTab==='function'&&!window.__fh_tab){var s=window.switchTab;window.switchTab=function(n){var r=s.apply(this,arguments);if(n==='istoricfull')setTimeout(renderFullHistory,0);if(n==='apihistory')setTimeout(renderApiHistory,0);if(n==='traininglab')setTimeout(renderTrainingLab,0);return r};window.__fh_tab=1;}
 ...
 ensureUI();
 loadExtraHistoryData().then(function(){try{if($('tab-apihistory')&&$('tab-apihistory').classList.contains('active'))renderApiHistory();if($('tab-istoricfull')&&$('tab-istoricfull').classList.contains('active'))renderFullHistory();if($('tab-traininglab')&&$('tab-traininglab').classList.contains('active'))renderTrainingLab();}catch(e){}});
}
...
window.renderApiHistory=function(){ensureUI();var top=$('apih-top'),root=$('apih-root');if(!top||!root)return;var s=window.API_HISTORY_SUMMARY||{};var valid=s.valid||{};var raw=s.raw||{};var leagues=Array.isArray(window.API_HISTORY_LEAGUES)?window.API_HISTORY_LEAGUES:[];var es=window.API_EVENTS_HISTORY_SUMMARY||{};if(!Object.keys(s).length&&!leagues.length&&!Object.keys(es).length){root.innerHTML='<div class="empty-state">Nu exista inca date API history.</div>';return;}top.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px">'+
 ...
 '<div class="metric"><div class="label">Sezoane indexate</div><div class="value">'+Number(es.seasons_indexed||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Meciuri numărate</div><div class="value">'+Number(es.total_events_counted||0).toLocaleString('ro-RO')+'</div></div>'+
 '</div><div style="font-size:12px;color:var(--muted);margin-top:10px">Catalog normalizat din /api/seasons/. Mai jos folosesc doar istoricul valid, iar indexul de evenimente îmi arată deja ce volum recent de meciuri există pentru dataset și training.</div>';
 var headEvents='';
 if(es&&Array.isArray(es.top_rows)&&es.top_rows.length){headEvents='<div style="margin-top:12px;font-size:13px;font-weight:800">Top sezoane indexate recent</div>'+es.top_rows.slice(0,10).map(function(r){return '<div class="list-item" style="margin-top:8px"><div style="font-weight:700">'+safe(r.league)+' • '+safe(r.season_name)+'</div><div style="font-size:12px;color:var(--muted)">An '+safe(r.year)+' • meciuri '+Number(r.events_count||0).toLocaleString('ro-RO')+' • sample '+Number(r.sample_count||0).toLocaleString('ro-RO')+'</div></div>';}).join('');}
 var cards=leagues.slice(0,20).map(function(r){return '<div class="list-item" style="margin-top:8px"><div style="font-weight:800">'+safe(r.league)+'</div><div style="font-size:12px;color:var(--muted)">Sezoane '+Number(r.seasons_count||0)+' • ani '+safe(r.first_year)+' → '+safe(r.last_year)+'</div><div style="font-size:12px;color:var(--muted)">Ultim sezon valid '+safe(r.latest_season_name||'—')+' • ID '+safe(r.latest_season_id||'—')+'</div></div>';}).join('');root.innerHTML=headEvents+(headEvents?'<div style="margin-top:14px;font-size:13px;font-weight:800">Ligi valide pentru lucru</div>':'')+cards||'<div class="empty-state">Nu exista ligi istorice.</div>';};
window.renderTrainingLab=function(){ensureUI();var top=$('train-top'),root=$('train-root');if(!top||!root)return;var ts=window.TRAINING_DATASET_SUMMARY||{};if(!Object.keys(ts).length){root.innerHTML='<div class="empty-state">Nu exista inca date pentru AI Training Lab.</div>';return;}var leagues=Array.isArray(ts.leagues)?ts.leagues:[];var markets=Array.isArray(ts.markets_ready)?ts.markets_ready:[];top.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px">'+
 '<div class="metric"><div class="label">Rows total</div><div class="value">'+Number(ts.rows_total||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Sezoane alese</div><div class="value">'+Number(ts.seasons_selected||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Sezoane încărcate</div><div class="value">'+Number(ts.seasons_loaded||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Sezoane eșuate</div><div class="value">'+Number(ts.seasons_failed||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Ligi</div><div class="value">'+Number(leagues.length||0).toLocaleString('ro-RO')+'</div></div>'+
 '<div class="metric"><div class="label">Piețe gata</div><div class="value">'+Number(markets.length||0).toLocaleString('ro-RO')+'</div></div>'+
 '</div><div style="font-size:12px;color:var(--muted);margin-top:10px">Acesta este primul strat al motorului de training. Datasetul este construit din meciuri istorice și targeturi gata pentru 1X2, BTTS, Over 1.5, Over 2.5 și Under 3.5.</div>';
 var blocks='';
 if(markets.length){blocks+='<div style="margin-top:12px;font-size:13px;font-weight:800">Piețe pregătite</div>'+markets.map(function(m){return '<div class="list-item" style="margin-top:8px"><div style="font-weight:700">'+safe(m)+'</div></div>';}).join('');}
 if(leagues.length){blocks+='<div style="margin-top:14px;font-size:13px;font-weight:800">Ligi din dataset</div>'+leagues.map(function(l){return '<div class="list-item" style="margin-top:8px"><div style="font-weight:700">'+safe(l)+'</div></div>';}).join('');}
 root.innerHTML=blocks||'<div class="empty-state">Datasetul există, dar nu are încă detalii afișabile.</div>';};
