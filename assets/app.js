...
function renderMatches(){
  ...
  if(wantedTypes && wantedTypes.length){
    filtered = filtered.map(function(m){
      if(!m.bestBet) return m;
      if(wantedTypes.indexOf(m.bestBet.type) >= 0) return m; // already matches
      if(!Array.isArray(m.eligibleCandidates)) return m;
      var matching = m.eligibleCandidates.find(function(c){ return c.bestBet && wantedTypes.indexOf(c.bestBet.type) >= 0; });
      if(!matching) return m;
      // Clone match with swapped bestBet — don't mutate original
  ...
  function renderCard(m){
    ...
    return '<div id="match-card-'+key+'" class="match-card match-card-v16 '+verdictClass+(MATCH_FOCUS_KEY === key ? ' match-card-focus' : '')+'">'+
      ...
      '<div class="m16-reco">'+
        ...
        '<div class="m16-reco-main">'+recLabel+'</div>'+
        m16ProbabilityHtml+
